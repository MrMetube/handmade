                ...
                if chunk != nil {
                    ...
                    for block := first_block; block != nil; block = block.next {
                        ...
                        for &source in entities {
                            ...
                            if len(region.entities) < cap(region.entities) {
                                dest := append_into(&region.entities)
                                ...
                                if entity_overlaps_rectangle(region.updatable_bounds, dest.p, dest.collision_volume) {
                                    dest.flags += { .active }
                                }
...
create_entity :: proc (region: ^SimRegion, id: EntityId) -> (result: ^Entity) {
    ...
    if len(region.entities) < cap(region.entities) {
        result = append_into(&region.entities)
    } else {
        unreachable()
    }
...
get_closest_traversable :: proc (region: ^SimRegion, from_p: v3, flags: bit_set[enum{ Unoccupied }] = {}) -> (result: TraversableReference, ok: bool) {
    ...
    for &test in region.entities {
        for point_index in 0 ..< cast(i64) len(test.traversables) {
...
entity_overlaps_rectangle :: proc (bounds: Rectangle3, entity_p: v3, entity_volume: Rectangle3) -> (result: b32) {
    result = intersects(bounds, add_offset(entity_volume, entity_p))
    return result
}

entity_overlaps_entity :: proc (a, b: ^Entity) -> (result: b32) {
    result = intersects(add_offset(a.collision_volume, a.p), add_offset(b.collision_volume, b.p))
    return result
}
...
get_or_add_brain :: proc (region: ^SimRegion, id: BrainId, kind: BrainKind) -> (result: ^Brain) {
    ...
    if result == nil {
        ...
        result = append_into(&region.brains)
        result ^= {
            id = id,
            kind = kind,
        }
...
can_collide :: proc (a, b: ^Entity) -> (result: b32) {
    if a != b {
        ...
        if .Collides in a.flags && .Collides in b.flags {
            if has_volume(a.collision_volume) && has_volume(b.collision_volume) {
                result = true
            }
        }
    }
