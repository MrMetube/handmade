        ...
        for arg in os.args[1:] {
            switch arg {
              ...
              case `AssetBuilder`: 
                ...
                run_command_or_exit(`C:\Odin\odin.exe`, args[:])
            }
        }
    }
...
build_platform :: proc() {
    ...
    if handle_running_exe_gracefully(debug_exe, .Skip) {
        ...
        if !run_command(`C:\odin\odin.exe`, args[:]) {
...
run_command :: proc(program: string, args: []string) -> (success: b32) {
    ...
    if win.CreateProcessW(win.utf8_to_wstring(program), win.utf8_to_wstring(joined_args), nil, nil, win.TRUE, 0, nil, working_directory, &startup_info, &process_info) {
        ...
        fmt.println("ERROR: Aborted to execute the command: %s %s with error %s", program, args, os.error_string(os.get_last_error()))
        success = false
    }
    return
}
