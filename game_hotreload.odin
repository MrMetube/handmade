...
init_game_lib :: proc(source_dll_name, temp_dll_name: string) -> (is_valid:b32, last_write_time: u64) {
	w_source_dll_name := win.utf8_to_wstring(source_dll_name)
	w_temp_dll_name := win.utf8_to_wstring(temp_dll_name)
	...
	win.CopyFileW(w_source_dll_name, w_temp_dll_name, false)
	last_write_time = get_last_write_time(source_dll_name)
	game_lib = win.LoadLibraryW(w_temp_dll_name)
